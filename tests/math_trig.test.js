...
async function runAllTests() {
  ...
  await runTest("Math Trig Module", async (assert) => {
    ...
    testAngles.forEach((angle) => {
      ...
      assert.ok(
        Math.abs(wasm.cos(angle) - Math.cos(angle)) < tolerance,
        `cos(${angle.toFixed(3)}) is accurate`,
      );
      assert.ok(
        Math.abs(wasm.sin(angle) - Math.sin(angle)) < tolerance,
        `sin(${angle.toFixed(3)}) is accurate`,
      );
    });
    ...
    assert.ok(
      Math.abs(Math.abs(wasm.normalize_angle(-PI)) - PI) < 0.001,
      "normalize_angle(-π) = ±π",
    );
    assert.ok(
      Math.abs(Math.abs(wasm.normalize_angle(3 * PI)) - PI) < 0.001,
