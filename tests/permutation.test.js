...
async function runTests() {
  ...
  for (const [input, log2n, expected] of bitReverseCases) {
    ...
    assert.strictEqual(result, expected, `bit_reverse(${input}, ${log2n}) = ${result}, expected ${expected}`);
  }
  ...
  for (const [input, numDigits, expected] of digitReverseCases) {
    ...
    assert.strictEqual(result, expected, `digitReverse4(${input}, ${numDigits}) = ${result}, expected ${expected}`);
  }
