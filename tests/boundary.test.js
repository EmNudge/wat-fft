...
describe("Boundary Conditions", async () => {
  ...
  for (const impl of implementations) {
    describe(`${impl.name}`, async () => {
      let wasm;

      // Load WASM before tests
      try {
        wasm = await loadWasm(impl.name);
        if (!wasm) {
          test.skip(`${impl.name} WASM not found`, () => {});
          return;
        }
      } catch (e) {
        test.skip(`${impl.name} failed to load: ${e.message}`, () => {});
        return;
      }
      ...
      describe("Non-power-of-2 sizes (edge cases)", () => {
        ...
        test("N=3 behavior is documented", () => {
          ...
          try {
            wasm[impl.fftFunc](3);
            // If it doesn't throw, the result is undefined for non-power-of-2
      ...
      describe("Extreme values", () => {
        ...
        test("Mixed positive and negative", () => {
          ...
          assert.ok(
            Math.abs(result.real[2] - 4) < 1e-10,
            `Nyquist should be 4, got ${result.real[2]}`,
          );
        });
      });
...
describe("Memory Layout Tests", async () => {
  ...
  if (!wasm) {
    test.skip("stockham WASM not found", () => {});
    return;
  }
  ...
  test("Memory size is sufficient for large FFTs", () => {
    ...
    // Each complex number is 16 bytes, N=8192 needs 128KB for data
    // Twiddles at offset 131072 need another ~128KB
  ...
  // Note: N=8192 requires more memory than currently allocated (4 pages = 256KB)
  // Stockham needs: data (128KB) + secondary (128KB) + twiddles (128KB) = 384KB minimum
  ...
  test("Memory limit documented: N=8192 would require 6+ pages", () => {
    ...
    // - Secondary buffer: 8192 * 16 = 131072 bytes (at offset 65536)
    // - Twiddles: 8192 * 16 = 131072 bytes (at offset 131072)
