...
const dependentModules = ['fft_main.wat', 'fft_fast.wat', 'fft_simd.wat', 'fft_radix4.wat'];
...
const originalModuleFiles = watFiles.filter(f => !['fft_fast.wat', 'fft_simd.wat', 'fft_radix4.wat'].includes(f));
...
fastModuleFiles.forEach(file => {
  const filePath = path.join(modulesDir, file);
  const content = fs.readFileSync(filePath, 'utf8');
  fastCombinedWat += content.trim().split('\n').map(line => `  ${line}`).join('\n') + '\n';
});
...
try {
  execSync(`wat2wasm ${fastOutputWatFile} -o ${fastOutputWasmFile}`);
...
simdModuleFiles.forEach(file => {
  const filePath = path.join(modulesDir, file);
  const content = fs.readFileSync(filePath, 'utf8');
  simdCombinedWat += content.trim().split('\n').map(line => `  ${line}`).join('\n') + '\n';
});
...
try {
  // Try with --enable-simd first, fall back to default
  try {
    execSync(`wat2wasm --enable-simd ${simdOutputWatFile} -o ${simdOutputWasmFile}`, { stdio: 'pipe' });
  } catch {
    execSync(`wat2wasm ${simdOutputWatFile} -o ${simdOutputWasmFile}`);
  }
...
radix4ModuleFiles.forEach(file => {
  const filePath = path.join(modulesDir, file);
  const content = fs.readFileSync(filePath, 'utf8');
  radix4CombinedWat += content.trim().split('\n').map(line => `  ${line}`).join('\n') + '\n';
});
...
try {
  try {
    execSync(`wat2wasm --enable-simd ${radix4OutputWatFile} -o ${radix4OutputWasmFile}`, { stdio: 'pipe' });
  } catch {
    execSync(`wat2wasm ${radix4OutputWatFile} -o ${radix4OutputWasmFile}`);
  }
