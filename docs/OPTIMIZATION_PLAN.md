...
- **Benchmark surface registry** (`benchmarks/shared/wat-surfaces.mjs`, 2026-07-06): the single source of truth for which wat implementations every bench (Node + browser) must measure; all bench files enumerate it instead of hand-picking modules, and `tests/benchmark-coverage.test.js` (in `test:all`) enforces registry ↔ dist ↔ bench-file consistency plus a reference-DFT correctness spot-check per entry. Added after discovering the browser rfft bench (and Node `bench:f32`/`bench:ifft32`) had silently kept benchmarking pre-Experiment-58/59 modules, reporting stale 14-35% losses to pffft. When a new module supersedes a flagship, update the registry and nothing else.
